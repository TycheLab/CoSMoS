...
author: "Filip Strnad, Simon Papalexiou, Yannis Markonis, Francesco Serinaldi"
date: "`r Sys.Date()`"
output: 
  rmarkdown::html_vignette:
    ...
    number_sections: yes
vignette: >
  %\VignetteIndexEntry{CoSMoS - A Complete Stochatic Modelling Solution}
...
their parameters can be found in [Section 4](#section_4). 
...
Another, more sophisticated approach is to use a function that creates the autocorrelation structure. This can be done by the `acs()` function or the user can create his own function. In the case of `acs()`, four types of 
...
The `stcs()` can produce values of the linear spatiotemporal correlation up to desired time lag and spatial distance according to this two classes, which comprise a variety of structures covering most of the cases of practical interest.
...
Similar to the one-dimensional case (i.e. the function ``generateTS()``), CoSMoS provides an easy to use function, ``generateMULTI()``, to simulate vectors of timeseries with specified (identical) marginals and spatiotemporal correlation function. Since the simulation relies on parent Gaussian Vector AutoRegressive (VAR) models, we need the preliminary computation of VAR parameters [(Biller and Nelson 2003)](https://doi.org/10.1145/937332.937333)  corresponding to the desired spatiotemporal correlation function of the target process, which done by the function ``fitVARMULTI()``. Therefore, the simulation is as follows
...
CoSMoS implement the simulation of spatially and temporally correlated random fields in two main functions called ``generateSTRF()`` and ``generateSTRFsepfast()``. ``generateSTRF()`` generates random fields with separable or nonseparable STCFs relying on the simulation of parent VAR processes similarly to ``generateMULTI()`` (see [Papalexiou (2018)](https://doi.org/10.1016/j.advwatres.2018.02.013) and [Papalexiou and Serinaldi (2020)](https://doi.org/10.1029/2019WR026331)). The function ``generateSTRFsepfast()`` generates RFs with approximately separable STCF [(Serinaldi and Kilsby 2018)](https://doi.org/10.1029/2018WR023055). As the algorithm in ``generateSTRFsepfast()`` is computationally less expensive than that implemented in ``generateSTRF()``, it allows the simulation of random fields over a larger number of grid points, at the cost of using separable STCFs and accepting some lack of accuracy in the exact reproduction of some terms of the required STCF [(Serinaldi and Kilsby 2018)](https://doi.org/10.1029/2018WR023055).
...
# Available distributions in CoSMoS package {#section_4}
